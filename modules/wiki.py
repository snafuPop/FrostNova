...
class Wiki(commands.Cog):
  ...
  async def dfopedia(self, ctx, *, search_terms: str = None):
    if search_terms is None:
      await ctx.send(embed = discord.Embed(title = "", description = "Try searching a page with `!dfopedia <search_term>`, {}.".format(ctx.author.mention)))
      return
    search_terms = titlecase(search_terms)
    term = search_terms.replace(" ", "%20")
    url = "http://wiki.dfo.world/api.php?action=parse&prop=wikitext&page={}&format=json".format(term.replace(" ","%20"))
    ...
    item = parse["parse"]
    url = "http://wiki.dfo.world/view/{}".format(item["title"].replace(" ", "_"))
    text = self.turn_into_dict(item) # split the page's contents into a dict to make it easily navigable

    # getting basic item information
    embed = discord.Embed(
      title = titlecase("**Level {} {} {}**".format(text["level"], text["rarity"], text["slot"])),
      description =
        self.exists_to_str(text, "weapon type") +
        self.exists_to_str(text, "armor weight class") +
        self.exists_to_str(text, "class") +
        self.exists_to_str(text, "binding") +
        self.exists_to_str(text, "set"),
    ...
    for param in ["weapon physical attack", "weapon magical attack", "independent attack"]:
      info += self.exists_to_str(text, param)
    if info:
    ...
    embed.set_footer(text = "Page ID: {}".format(item["pageid"]))
  ...
  def exists_to_str(self, dict, key, *, plus: bool = False, percent: bool = False):
    ...
    if key in dict and dict[key]:
      if key == "set":
        ...
        output = "**{}:** {}{}{}\n".format(self.replace[key] if key in self.replace else titlecase(key), "+" if plus else "", titlecase(dict[key]), "%" if percent else "")
  ...
  def format_desc(self, dict, param):
    ...
    if param == "description" and param in dict:
      output = dict["description"].replace("\n\n", "\n").replace("<br>", "\n").replace("{{IconLink|", "").replace("}}", "").replace("[", "").replace("]", "")
    if param == "flavor text" and param in dict:
      output = dict["flavor text"].replace("\n\n", "\n").replace("<br>", "\n").replace("{{IconLink|", "").replace("}}", "").replace("[", "").replace("]", "")
