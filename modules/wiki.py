...
async def dfopedia(ctx, *, search_terms: str=None):
  ...
  if search_terms is None:
    # catches null-answer
    embed = discord.Embed(title ="", description = "Try asking a question with `!dfopedia <search_term>`, {}".format(ctx.message.author.mention))
...
@bot.command(pass_context=True, description = "Pulls a page from the Official Stardew Valley Wiki")
async def stardew(ctx, *, search_terms: str=None):
  '''Searches the DFO Wiki.'''

  if search_terms is None:
    # catches null-answer
    embed = discord.Embed(title ="", description = "Try asking a question with `!stardew <search_term>`, {}".format(ctx.message.author.mention))
    await bot.say(embed = embed)
