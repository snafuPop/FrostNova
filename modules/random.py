...
class Random(commands.Cog):
  ...
  @commands.command(description = "Generates a random number from 1 to a defined number (by default 100)")
  ...
  @commands.command(description = "Chooses from a list of options")
  async def choose(self, ctx, *choices):
    await ctx.send(embed = discord.Embed(title = "", description = "I need more than one option. Try `{}choice <option1> <option2> ...`, {}".format(ctx.prefix, ctx.author.mention)) if len(choices) < 2 else discord.Embed(title = "", description = "I choose **{}**, {}.".format(choice(choices), ctx.author.mention), color = ctx.author.color))
