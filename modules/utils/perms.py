...
def is_owner_check(message):
    ...
    return message.author.id == '94236862280892416'
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ('reshiram cheater', 'tapu lele'), **perms)

    return commands.check(predicate)

def admin_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name == 'tapu lele', **perms)
...
def is_lounge_cpp():
    return is_in_servers('330936821766684672')